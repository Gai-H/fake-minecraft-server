...
impl S2CStatusResponsePacket {
    ...
    fn get_response_json() -> String {
        let version_name = CONFIG.get::<String>("version_name").unwrap_or(Self::DEFAULT_VERSION_NAME.into());
        let version_protocol = CONFIG.get::<u16>("version_protocol").unwrap_or(Self::DEFAULT_VERSION_PROTOCOL);
        let description = CONFIG.get::<String>("description").unwrap_or(Self::DEFAULT_DESCRIPTION.into());
        let players_max = CONFIG.get::<u16>("players_max").unwrap_or(Self::DEFAULT_PLAYERS_MAX);
        let players_online = CONFIG.get::<u16>("players_online").unwrap_or(Self::DEFAULT_PLAYERS_ONLINE);
